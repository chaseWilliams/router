...
class App < Sinatra::Application
  ...
  get '/' do
    ...
    begin
      ...
      weather = JSON.parse RestClient.get 'http://localhost:3002/?' + "mode=coordinates&lat=#{lat}&lon=#{lon}"
      wiki_article = JSON.parse RestClient.get 'http://localhost:3003/?' + "lat=#{lat}&lon=#{lon}"
      puts wiki_article
      flickr_url = JSON.parse RestClient.get 'http://localhost:3001/?' + "temp=#{weather['data']['temp']}" +
